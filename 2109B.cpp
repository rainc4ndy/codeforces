...
void solve() {
    ...
    while (n > 1 || m > 1) {
        ...
        ll costRowCut = (n2 > 1 ? 64 - __builtin_clzll(n2 - 1) : 0)
                       + (m  > 1 ? 64 - __builtin_clzll(m  - 1) : 0);
        ll costColCut = (n  > 1 ? 64 - __builtin_clzll(n  - 1) : 0)
                       + (m2 > 1 ? 64 - __builtin_clzll(m2 - 1) : 0);

        if (costRowCut < costColCut) {
            n = n2;
        } else {
            m = m2;
        }
